...
func parseToLinuxRouteStruct(output []byte) (linuxRouteStruct, error) {
	...
	for scanner.Scan() {
		...
		//	IRTT:        tokens[10],
		//}
		//fmt.Println("tmp:",tmp.Gateway,tmp.Iface,tmp.Destination)
...
func parseToLinuxRouteStructByIfIP(ipStr string,output []byte) (linuxRouteStruct, error) {
	// parseLinuxProcNetRoute parses the route file located at /proc/net/route
	// and returns the IP address of the default gateway. The default gateway
	// is the one with Destination value of 0.0.0.0.
	//
	...
	for scanner.Scan() {
		...
		//fmt.Println("tmp:",tmp.Gateway,tmp.Iface,tmp.Destination)
		interFace,err := net.InterfaceByName(tmp.Iface)
		if err != nil{
			return linuxRouteStruct{},nil
		}
		interAddrs,err := interFace.Addrs()
		if err != nil{
			return linuxRouteStruct{},nil
		}
		//jlog.Debug(interAddrs)
		for _,v := range interAddrs{
			if strings.Split(v.String(),"/")[0] == ipStr{
				return tmp,nil
			}
		}
		//return linuxRouteStruct{
...
func parseLinuxGatewayIPByIfIP(ipStr string,output []byte) (net.IP, error) {

	parsedStruct, err := parseToLinuxRouteStructByIfIP(ipStr,output)
	if err != nil {
		return nil, err
	}
